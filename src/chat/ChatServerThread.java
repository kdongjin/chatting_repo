...
public class ChatServerThread extends Thread {
	...
	public void run() {		
		...
		// 소켓을 통해 읽기 기능을 부여(InputStream - InputStreamReader - BufferedReader)
		// 대화를 1바이트를 -> 2바이트 방식으로 변환 , 버퍼장치를 통해서 속도를 개선
		InputStream inputStream = null;
		InputStreamReader inputStreamReader = null;
		BufferedReader bufferedReader = null;
		
		// 2. 소켓을 통해 출력 기능을 부여(OutputStream - OutputStreamWriter - BufferedWriter)
		// 2바이트를 1바이트로 전환해서 출력하는기능 , 버퍼장치를 통해서 속도롤 개선
		OutputStream outputStream = null;
		OutputStreamWriter outputStreamWriter = null;
		PrintWriter printWriter = null; // 속도보다는 쓰기 편리하게 BufferedWriter가 아닌 'PrintWriter' 사용
		
		try {
			//3. socket 클라이언트 정보를 갖고 있는 소켓이다.
			//소켓을 통해서 클라이언트부터 오는 데이타를 받기위함 스트림 설정
			...
			//소켓을 통해서 클라이언트에게 메세지를 전달할 스트림설정
			...
			while(true) {
				/* 2-0. 상대방이 엔터치는 것까지 다 인식(= scanner.nextLine()), 
				 * 클라이언트부터 데이가 입력 받을 때까지 무한 대기.
				...
				if (messageReceive == null) {
					...
					// 2) 클라이언트가 연락을 끊었음을 모든 다른 클라이언트에게 알림 전송
				...
				if("join".equals(tokens[0])) {
					...
					// 4) 클라이언트가 입장했음을 모든 다른 클라이언트에게 입장 알림 전송
