...
public class ChatServer {
	...
	//1. 서버 포트 번호 설정(1024~65535) 상수지정
	public static final int PORT = 1111;
	public static void main(String[] args) {
		
		String ip = null;
		//2. 서버IP주소 확인(InetAddress)  (172.30.1.251) 주소를 가져온다. 
		try {
		...
		/* 3.  여러 클라이언트와의 여러 스트림(흐름)을 리스트화하여 저장
		...
		try {
			...
			// 5. 서버 소켓 바인딩(소켓에 IP(전화번호), port 번호를 부여)(집전화기 : 전화번호 부여함)
			serverSocket.bind(new InetSocketAddress(ip, PORT));
			//메인스레드 아이피주소(전화번호) 포트번호
